...
test('registration with a valid referrer nickname records the referral and grants the registrant +5', function (): void {
    ...
    expect(creditsOfCouple(activeCoupleOf($referred)->id))->toBe(5);
    // The referrer is not paid yet — that waits for the referred user's first open.
    expect(creditsOfCouple(activeCoupleOf($referrer)->id))->toBe(0);
});

test('registration without a referrer nickname creates no referral and leaves credits 0', function (): void {
